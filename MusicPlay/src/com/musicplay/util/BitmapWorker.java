...
public class BitmapWorker {
	...
	public void fetch(String path, ImageView imageView) {
		WeakReference<BitmapWorkerTask> oldWeakTask = (WeakReference<BitmapWorkerTask>) imageView
				.getTag();
		// 通过ImageView对象的Tag属性判断当前ImageView是不是AdapterView中重用的子项视图的控件
		// Tag是一个加载图片的异步任务
		if (oldWeakTask != null) {
			// 先取到图片加载异步任务的实际对象
			BitmapWorkerTask task = oldWeakTask.get();
			if (task != null) {
		...
		WeakReference<BitmapWorkerTask> newWeakTask = new WeakReference<BitmapWorker.BitmapWorkerTask>(task);
		...
		protected void onPostExecute(Bitmap result) {
			...
			if (imageView != null) {
				if(result != null){
					...
					//设置默认封面
					imageView.setBackgroundResource(R.drawable.ic_default_art);
				}
			}
